...
//    if(D.fieldType==Split) compensateCurrent_Split_umeda(&D,iteration);
//    else if(D.fieldType==Yee) compensateCurrent_Yee_umeda(&D,iteration);
//	 else ;
...
void compensateCurrent_Split_umeda(Domain *D,int iteration)
{
	...
	double inv2Pi,r,dr;
	double VR[D->numMode],VI[D->numMode];
   ...
   jstart=D->jstart;    jend=D->jend;
	...
	for(i=istart; i<iend; i++)
		for(j=jstart+1; j<jend; j++)	
		{
			...
			m=0;
				VR[m]=inv2Pi/(r*dr)*((r+0.5)*(D->PrCR[m][i][j]-D->PlCR[m][i][j])-(r-0.5)*(D->PrCR[m][i][j-1]-D->PlCR[m][i][j-1]));
				VI[m]=0.0;
			for(m=1; m<numMode; m++) {
				VR[m]=inv2Pi/(r*dr)*((r+0.5)*(D->PrCR[m][i][j]-D->PlCR[m][i][j])-(r-0.5)*(D->PrCR[m][i][j-1]-D->PlCR[m][i][j-1]))
					+inv2Pi*m/(2*r*dr)*(D->SrCI[m][i][j]+D->SrCI[m][i][j-1]-D->SlCI[m][i][j]-D->SlCI[m][i][j-1]);
				VI[m]=inv2Pi/(r*dr)*((r+0.5)*(D->PrCI[m][i][j]-D->PlCI[m][i][j])-(r-0.5)*(D->PrCI[m][i][j-1]-D->PlCI[m][i][j-1]))
					-inv2Pi*m/(2*r*dr)*(D->SrCR[m][i][j]+D->SrCR[m][i][j-1]-D->SlCR[m][i][j]-D->SlCR[m][i][j-1]);
			}
...
/*
	...
	{
		m=0;
			VR[m]=inv2Pi/(0.5*dr)*(D->PrCR[m][i][j]-D->PlCR[m][i][j]);
			VI[m]=0.0;
		for(m=1; m<numMode; m++) {
			VR[m]=inv2Pi/(0.5*dr)*(D->PrCR[m][i][j]-D->PlCR[m][i][j]);
				-inv2Pi*m/dr*(D->SrCI[m][i][j]-D->SlCI[m][i][j]);
			VI[m]=inv2Pi/(0.5*dr)*(D->PrCI[m][i][j]-D->PlCI[m][i][j]);
				+inv2Pi*m/dr*(D->SrCR[m][i][j]-D->SlCR[m][i][j]);
		}
            ...
            factor=vp*weight*coeff[s]/(2.0*j1+1.0);
	         if(j1==0) {
	           ...
	           if(D->currentCons==Lifschitz) {
                for(m=1; m<numMode; m++) { 
                  D->JpR[m][i1][j1+jstart]+=factor*coss[m]*alpha;
                  D->JpI[m][i1][j1+jstart]-=factor*sins[m]*alpha;
                }
              ...
              D->JpR[0][i2][j2+jstart]+=factor;
   	        if(D->currentCons==Lifschitz) {
                for(m=1; m<numMode; m++) { 
                  ...
                  D->JpI[m][i2][j2+jstart]-=factor*sins[m]*alpha;
		          }					 
              } else if(D->currentCons==Davidson) {
				    //delta function and weights
...
//step 1 --------------------------------------------------------------
            ...
            factor=vp*weight*coeff[s]/(2.0*j1+1.0);
	         if(j1==0) {
              tmpP[0][0]=Wz[0]*Wr[0]*factor*center;
              tmpP[1][0]=Wz[1]*Wr[0]*factor*center;
              tmpP[0][1]=Wz[0]*Wr[1]*factor;
              tmpP[1][1]=Wz[1]*Wr[1]*factor;
              for(ii=0; ii<2; ii++)
                for(jj=1; jj<2; jj++)
