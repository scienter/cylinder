...
//    filter(D,D->PlCR,D->PlCI);
//    filter(D,D->SlCR,D->SlCI);
    filter(D,D->BzCR,D->BzCI);
    filter(D,D->EzCR,D->EzCI);
  } else ;
...
void solve_Split(Domain *D,int iteration)
{
  ...
  if(D->filter==ON && iteration%D->filterIter==0) {
//    filter(D,D->PlR,D->PlI);
//    filter(D,D->SlR,D->SlI);
    filter(D,D->BzR,D->BzI);
    filter(D,D->EzR,D->EzI);
  } else ;
