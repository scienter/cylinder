...
void checkEnergyCons(Domain *D,int iteration)
{
	...
	sumF=0.0;
 	factor=M_PI*D->dr*D->dr*D->dz*D->lambda*D->lambda*D->lambda;		
	for(i=istart; i<iend; i++)
		for(j=jstart; j<jend; j++)
		{
			r=2*(j-jstart)+1;
			...
			Ez=0.25*(D->EzR[m][i][j]+D->EzR[m][i+1][j]+D->EzR[m][i][j+1]+D->EzR[m][i+1][j+1]);
			Br=(D->SlR[m][i][j]-D->SrR[m][i][j])*0.5;
			Bp=(D->PrR[m][i][j]-D->PlR[m][i][j])*0.5;
			Bz=0.25*(D->BzR[m][i][j]+D->BzR[m][i+1][j]+D->BzR[m][i][j+1]+D->BzR[m][i+1][j+1]);
			sumF+=2*(Er*Er+Ep*Ep+Ez*Ez+Br*Br+Bp*Bp+Bz*Bz)*r*factor;
			for(m=1; m<D->numMode; m++) {
				...
				Ez=0.25*(D->EzI[m][i][j]+D->EzI[m][i+1][j]+D->EzI[m][i][j+1]+D->EzI[m][i+1][j+1]);
				Br=(D->SlI[m][i][j]-D->SrI[m][i][j])*0.5;
				Bp=(D->PrI[m][i][j]-D->PlI[m][i][j])*0.5;
				Bz=0.25*(D->BzI[m][i][j]+D->BzI[m][i+1][j]+D->BzI[m][i][j+1]+D->BzI[m][i+1][j+1]);
				sumF+=(Er*Er+Ep*Ep+Ez*Ez+Br*Br+Bp*Bp+Bz*Bz)*r*factor;
			}
		}
